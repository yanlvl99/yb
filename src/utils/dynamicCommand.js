...
exports.dynamicCommand = async (paramsHandler) => {
  ...
  if (isActiveAntiLinkGroup(remoteJid) && isLink(fullMessage)) {
    ...
    if (!(await isAdmin({ remoteJid, userJid, socket }))) {
        // Envia uma mensagem avisando que o usuário não pode enviar links
        await sendReply(
            "Anti-link ativado! Você somente admins podem enviar links neste grupo."
        );

        // Remove a mensagem que contém o link
